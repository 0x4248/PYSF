    ...
    while True:
        try:
            if script == "":
                ...
                cmdline = input("PYSF Script("+Fore.CYAN+script+Style.RESET_ALL+")>")
            ...
            if cmdline.startswith("use"):
                if os.path.isdir(cmdline[4:]):
                    ...
                    verbose.minus("DIR does not exist")
                    continue
                if os.path.isfile(cmdline[4:]+"/PYSFSCRIPT"):
                    verbose.plus("Set script to "+cmdline[4:])
                else:
                    verbose.minus("DIR exists but no PYSFSCRIPT file found")
            ...
            if cmdline.startswith("info"):
                if script == "":
                    verbose.minus("No script set")
                    continue
                else:
                    with open(script+"/info.json", 'r') as infofile:
                        infojson = json.load(infofile)
                    print(infojson["name"])
                    print(infojson["description"])
                    try:
                        print("Requires:"+infojson["packages"])
                    except:
                        print("No requirements")
                    print("ARG\t\t|REQUIRED\t|DEFAULT")
                    for i in range(len(infojson["args"])):
                        print(str(infojson["args"][i])+"\t\t|"+str(infojson["required"][i])+"\t\t|"+str(infojson["defaults"][i]))
                ...
                for i in args:
                    if i.startswith(cmd[1]):
                        ...
                        if confirm.upper() == "Y":
                            ...
                            verbose.minus("Canceled overwrite")
                            break
            ...
            if cmdline == "clearargs":
                ...
                if confirm.upper() == "Y":
                    args = ""
                    verbose.plus("Cleared args")
                else:
                    verbose.info("Canceled operation")
            if cmdline == "run" or cmdline == "execute":
                if script == "":
                    verbose.minus("No script is set")
                    continue
                try:
                    script_module = importlib.import_module(script.replace("/",".").replace("\\","."))
                    ...
                    if script_return == None:
                        ...
                        verbose.plus("Script ended successfully (Code 0)")
                    elif script_return == 1:
                        verbose.minus("Script ended unsuccessfully (Code 1)")
            ...
            if cmdline == "create":
                ...
                print("Enter name of first argument leave blank to not create any arguments")
                script_args = []
                defaults = []
                require = []
                while True:
